...
app.post(process.env.GRAPHQL_CHAT_BOT_TX_ZONA_CAFETERA_WEBHOOK_HTTP_END_POINT_POST, (req, res) =>{
    console.log("LLEGA RQST ===> ", JSON.stringify(req.body))
    for (let index = 0; index < ((req.body.entry || [])).length; index++) {
        const entry = ((req.body.entry || []))[index];
        for (let indexEntry = 0; indexEntry < entry.changes.length; indexEntry++) {
            const change = entry.changes[indexEntry];
            broker.forward$("ClientBotLink", "clientgateway.graphql.mutation.ClientBotTxPlusZonaCafeteraLinkMessageReceived",{ args: change.value }).subscribe(res => {
                console.log("RES ===> ", res)
            })
